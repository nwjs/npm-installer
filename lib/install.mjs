...
/**
 * URL to download or get binaries.
 */
let url = '';
...
if (parsedUrl.protocol === 'file:') {
  ...
  if (filePath.slice(-3) === '.zip') {
    compressing.zip.uncompress(filePath, '.')
      .then(async () => {
        await rm(filePath, {
          recursive: true,
          force: true,
    ...
    compressing.tgz.uncompress(filePath, '.')
      .then(async () => {
        await rm(filePath, {
          recursive: true,
          force: true,
  ...
  get(url, (response) => {
    ...
    response.on('end', () => {
      if (PLATFORM_KV[hostOs] === 'linux') {
        compressing.tgz.uncompress(nwDir, '.')
          .then(async () => {
            await rm(nwDir, {
              recursive: true,
              force: true,
        ...
        compressing.zip.uncompress(nwDir, '.')
          .then(async () => {
            await rm(nwDir, {
              recursive: true,
              force: true,
