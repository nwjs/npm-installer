...
if (parsedUrl.protocol === 'file:') {
  ...
  if (filePath.slice(-3) === '.zip') {
    compressing.zip.uncompress(filePath, '.').then(async () => await rmAndRename());
    // If the compressed file is of TGZ format:
  } else if (filePath.slice(-7) === '.tar.gz') {
    compressing.tgz.uncompress(filePath, '.').then(async () => await rmAndRename());
  } else {
  ...
  get(url, (response) => {
    ...
    response.on('end', () => {
      if (PLATFORM_KV[hostOs] === 'linux') {
        compressing.tgz.uncompress(nwDir, '.').then(async () => await rmAndRename());
      } else {
        compressing.zip.uncompress(nwDir, '.').then(async () => await rmAndRename());
      }
    });
